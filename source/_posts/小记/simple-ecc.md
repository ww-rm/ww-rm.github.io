...
- 模逆运算: 指求解方程 $ax \equiv 1 \bmod p$, 即 $x$ 在模 $p$ 运算下的"倒数".
...
根据前面 ECDLP 的定义, 定义用户的私钥为 $d$, 公钥为 $Q$, 且 $Q = dG$, $G$ 是循环群 $<G, n>$ 的生成元. 其余所有和椭圆曲线有关的参数均是公开已知, 只有 $d$ 的值是秘密保留的, 当 $p$ 足够大时, 求解 $d$ 是困难的.
